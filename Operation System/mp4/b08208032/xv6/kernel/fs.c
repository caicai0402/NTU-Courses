...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDOUBLEINDIRECT){
    ...
    if(bn < NDOUBLEINDIRECT / 2){
      ...
      bn -= NDOUBLEINDIRECT / 2;
    }

    uint bn1 = bn / NADDREESS;
    struct buf *bp1 = bread(ip->dev, addr);
    uint *a1 = (uint*)bp1->data;
    if((addr = a1[bn1]) == 0){
      a1[bn1] = addr = balloc(ip->dev);
      log_write(bp1);
    }
    brelse(bp1);

    uint bn2 = bn % NADDREESS;
...
itrunc(struct inode *ip)
{
  ...
  for(k = 1; k <= 2; k++) {
    if(ip->addrs[NDIRECT+k]) {
      struct buf *bp1 = bread(ip->dev, ip->addrs[NDIRECT+k]);
      uint *a1 = (uint *)bp1->data;
      for(i = 0; i < NADDREESS; i++) {
        if(a1[i]) {
          struct buf *bp2 = bread(ip->dev, a1[i]);
          uint *a2 = (uint *)bp2->data;
          for(j = 0; j < NADDREESS; ++j) {
            if(a2[j]) 
              bfree(ip->dev, a2[j]);
          }
          brelse(bp2);
