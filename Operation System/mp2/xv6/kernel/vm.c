...
void vmprint_tree(pagetable_t pagetable, uint64 vm, int level){
  if(0 <= level && level <= 2){
    static char prefix[3][9] = {"        ", "    ", ""};
    
    int last = 511;
    while(!((pagetable[last] & PTE_V) || (pagetable[last] & PTE_S)))
      last --;

    for(int i = 0; i <= last; i++){
      if((pagetable[i] & PTE_V) || (pagetable[i] & PTE_S)){
        printf("%s+-- %d: pte=%p va=%p", prefix[level], i, &pagetable[i], vm + ((uint64)i << (PGSHIFT + 9 * level)));
        ...
        if(pagetable[i] & PTE_V) printf(" V");
        if(pagetable[i] & PTE_R) printf(" R");
        if(pagetable[i] & PTE_W) printf(" W");
        if(pagetable[i] & PTE_X) printf(" X");
        if(pagetable[i] & PTE_U) printf(" U");
        if(pagetable[i] & PTE_S) printf(" S");
        printf("\n");

        if(level == 2)
          prefix[0][0] = prefix[1][0] = i != last? '|' : ' ';

        vmprint_tree((pagetable_t)PTE2PA(pagetable[i]), vm + ((uint64)i << (PGSHIFT + 9 * level)), level - 1);
      }
    }
  }
  return ;
